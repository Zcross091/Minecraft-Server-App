...
export class ServerEngine {
  ...
  async detectNetworkIp() {
    ...
    // 2. Fetch device's real WAN/Internet Public IP
    try {
      ...
      // Fallback to secondary provider or keep existing IP
      try {
        if (typeof fetch === 'function') {
  ...
  saveState() {
    try {
      // Save max 50 logs to localStorage to preserve storage quota while keeping memory buffer intact
  ...
  updateConfig(newConfig) {
    ...
    if (newConfig.serverFolder !== undefined) {
      safeConfig.serverFolder = stripHtml(newConfig.serverFolder).trim() || '/sdcard/Download/MinecraftServers/SMP';
    }
    ...
    if (newConfig.motd !== undefined) {
      safeConfig.motd = stripHtml(newConfig.motd).trim() || 'A Crossplay Minecraft SMP Server';
    }
  ...
  createWorldBackup(customName) {
    ...
    const name = customName ? (stripHtml(customName) || `World Backup (${timeStr})`) : `World Backup (${timeStr})`;
