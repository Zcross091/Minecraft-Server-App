...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnCreateBackup) {
    btnCreateBackup.addEventListener('click', () => {
      const name = prompt('Enter a name for this world backup snapshot:', `iOS_Backup_${new Date().toLocaleDateString().replace(/\//g,'-')}`);
      if (name !== null) {
        serverEngine.createWorldBackup(name);
        alert('iOS World backup snapshot created and saved!');
      }
    });
  }
  ...
  if (btnSaveProperties) {
    btnSaveProperties.addEventListener('click', () => {
      ...
      serverEngine.updateConfig({
        ...
        onlineMode: settingOnlineMode ? !settingOnlineMode.checked : true,
        ...
        motd: settingMotd ? settingMotd.value.trim() : 'An iOS Crossplay Minecraft SMP Server'
      });
  ...
  if (btnConfirmClaimed) {
    btnConfirmClaimed.addEventListener('click', () => {
      ...
      if (badgePlayitStatus) {
        ...
        badgePlayitStatus.style.background = 'rgba(48, 209, 88, 0.2)';
        badgePlayitStatus.style.color = '#30d158';
      }
      alert(`🎉 Success! Playit Crossplay Tunnel is now ACTIVE!\n\nGlobal Address: ${newDomain}\n\nFriends from anywhere in the world can now join your iOS server!`);
    });
  }
  ...
  function generateInviteText() {
    ...
    return `🎮 JOIN MY iOS MINECRAFT SMP CROSSPLAY SERVER! 🎮

🏠 1. SAME HOME WI-FI (Instant / Zero-Lag):
• Java Edition (PC/Mac/Linux): ${localIp}:${javaPort}
• Bedrock (iOS/Android/Xbox/Switch/Win10): ${localIp} | Port: ${bedrockPort}
